...
$(document).ready(function() {
    ...
    $(document).on("click", '.exitTVMode', function(e) {
        ...
        window.location.replace("./dashboard.php#mainDashboard");
    });
        ...
        $('#page-content').load('inc/pages/monitorDashboard.php?monitorID=' + monitorID, function() {
            ...
            $.ajax({
                ...
                success: function(theResponse) {
                    ...
                    var monitorURL = json[0]['monitorURL'];
                    var monitorType = json[0]['monitorType'];
                    var monitorAlertContacts = json[0]['monitorAlertContacts'];
                    var monitorPort = "";
                    if (monitorType == 6) {
                        ...
                        monitorPort = ":" + json[0]['monitorPort'];
                    }
    ...
    $(document).on("change", '#newMonitorType', function(e) {
        ...
        if ($("#newMonitorType").val() == 5) {
            $('.validateIgnore').removeClass('validateIgnore');
            $('#newHBMonitor :input:hidden').addClass('validateIgnore');
            if ($("#newHTTPMonitor").is(":visible")) {
                $("#newHTTPMonitor").slideUp("slow", function() {
                    $("#newHBMonitor").slideDown("slow");
                });
            } else if ($("#newKeywordMonitor").is(":visible")) {
                $("#newKeywordMonitor").slideUp("slow", function() {
                    $("#newHBMonitor").slideDown("slow");
        ...
        if ($("#newMonitorType").val() == 6) {
            $('.validateIgnore').removeClass('validateIgnore');
            $('#newTMMonitor :input:hidden').addClass('validateIgnore');
            if ($("#newHTTPMonitor").is(":visible")) {
                $("#newHTTPMonitor").slideUp("slow", function() {
                    $("#newTMMonitor").slideDown("slow");
                });
            } else if ($("#newKeywordMonitor").is(":visible")) {
                $("#newKeywordMonitor").slideUp("slow", function() {
                    $("#newTMMonitor").slideDown("slow");
            ...
            $('.tmMainContainer.tmNewMonitor label.errorText').css("display", "none");
            $('.tmMainContainer.tmNewMonitor .result .tmResults').hide();
            if ($('.tmMainContainer.tmNewMonitor .commandSelectionToolDock .commandSelectionTool').length) {
                $('.tmMainContainer.tmNewMonitor .selectedCommands ul').html("");
                $('.tmMainContainer.tmNewMonitor .commandSelectionToolDock .commandSelectionTool').detach().appendTo('.tmMainContainer.tmNewMonitor .selectedCommands ul');
            }
            $("input[name='newTMMonitorFriendlyName']").focus().setCursorToTextEnd();
        }
    ...
    function keepAlertContactValuesEdit(theValues) {
        ...
        for (var i = 0; i < monitorAlertContactsArray.length; i++) {
            var monitorAlertContactsArray2 = monitorAlertContactsArray[i].split(';');
            $('#editMonitorAlertContacts input:checkbox[value="' + monitorAlertContactsArray2[0] + '"]').attr('checked', 'checked');
            $('.monitorAlertThresholdEdit' + monitorAlertContactsArray2[0]).val(monitorAlertContactsArray2[1]).prop("disabled", false);
            $('.monitorAlertRecurrenceEdit' + monitorAlertContactsArray2[0]).val(monitorAlertContactsArray2[2]).prop("disabled", false);
            if ((monitorAlertContactsArray2[1] != 0 || monitorAlertContactsArray2[2] != 0) && $('.alertContactsSettingsStatus').val() == 1 && monitorAlertContactsArray2[1] && monitorAlertContactsArray2[2]) {
                $('.monitorAlertContactsAdvancedOptionsEdit' + monitorAlertContactsArray2[0]).show();
            }
        }
    ...
    $(document).on("click", '.editMonitor', function(e) {
        ...
        $('#editMonitorForm').attr('action', 'inc/dml/monitorDML.php?action=editMonitor&monitorID=' + monitorID);
                ...
                $('#editMonitorAlertContacts').html(theResponse).promise().done(function() {
                    $.ajax({
                        ...
                        success: function(theResponse) {
                            var json = eval(theResponse);
                            var monitorType = json[0]['monitorType'];
                            var monitorFriendlyName = fRsi(json[0]['monitorFriendlyName']);
                            var monitorURL = fRsi(json[0]['monitorURL']);
                            var monitorSubType = json[0]['monitorSubType'];
                            var monitorKeywordValue = fRsi(json[0]['monitorKeywordValue']);
                            var monitorKeywordType = json[0]['monitorKeywordType'];
                            var monitorKeywordCaseType = json[0]['monitorKeywordCaseType'];
                            var monitorPort = json[0]['monitorPort'];
                            var monitorHTTPUsername = fRsi(json[0]['monitorHTTPUsername']);
                            var monitorHTTPPassword = fRsi(json[0]['monitorHTTPPassword']);
                            var monitorInterval = json[0]['monitorInterval'];
                            var monitorGracePeriod = json[0]['monitorGracePeriod'];
                            var monitorTimeout = json[0]['monitorTimeout'];
                            var monitorAlertContacts = json[0]['monitorAlertContacts'];
                            ...
                            var nextMonitorID = (json[0].hasOwnProperty('closestMonitorLinks') && json[0]['closestMonitorLinks'].hasOwnProperty('nextMonitorID')) ? json[0]['closestMonitorLinks']['nextMonitorID']['monitorID'] : null;
                            var previousMonitorID = (json[0].hasOwnProperty('closestMonitorLinks') && json[0]['closestMonitorLinks'].hasOwnProperty('previousMonitorID')) ? json[0]['closestMonitorLinks']['previousMonitorID']['monitorID'] : null;
    ...
    $('#newProSMSAlertContactCountry').change(function() {
        var selected = $(this).find('option:selected');
        var countryCode = selected.data('country');
        $('#newProSMSAlertContactCountryCode').val(countryCode);
    });
    $('#newProVoiceCallAlertContactCountry').change(function() {
        var selected = $(this).find('option:selected');
        var countryCode = selected.data('country');
        $('#newProVoiceCallAlertContactCountryCode').val(countryCode);
    });
    $("#newAlertContactType").on("change", function() {
    ...
    function changeAlertContactForm(selectedForm) {
        var alertContactHolders = ["newMobileFreeAlertContact", "newEmailAlertContact", "newTwitterAlertContact", "newBoxcarAlertContact", "newWebHookAlertContact", "newPushbulletAlertContact", "newZapierAlertContact", "newProSMSAlertContact", "newPushoverAlertContact", "newHipchatAlertContact", "newSlackAlertContact", "newProVoiceCallAlertContact", "newSplunkAlertContact", "newPagerDutyAlertContact", "newOpsGenieAlertContact", "newTelegramAlertContact", "newGoogleAdWordsAlertContact", "newMSTeamsAlertContact", "newHangoutsChatAlertContact", "newFacebookAdsAlertContact", "newDiscordAlertContact"];
        for (var i = 0; i < alertContactHolders.length; i++) {
            if (alertContactHolders[i] == selectedForm || !$("#" + alertContactHolders[i]).is(':visible')) {
                continue;
            }
            $("#" + alertContactHolders[i]).slideUp("slow ");
        }
        $("#" + selectedForm).slideDown("slow");
    }
    $("#newMobileFreeProviderID").on("change", function() {
        var currentNewMobileFreeProviderID = $(this).val();
        if (currentNewMobileFreeProviderID && currentNewMobileFreeProviderID != 0) {
            $.ajax({
                ...
                success: function(theResponse) {
                    ...
                    var mobileProviderSuffix = json[0]['mobileProviderSuffix'];
                    var mobileProviderNumberLimit = json[0]['mobileProviderNumberLimit'];
                    var mobileProviderNumberLimitUp = json[0]['mobileProviderNumberLimitUp'];
                    if (mobileProviderPrefix) {
                        $('#newMobileFreeAlertContactPrefix').html(mobileProviderPrefix + ' ');
                    } else {
                        $('#newMobileFreeAlertContactPrefix').html('');
                    }
                    if (mobileProviderSuffix) {
                        $('#newMobileFreeAlertContactSuffix').html(' ' + mobileProviderSuffix);
                    } else {
                        $('#newMobileFreeAlertContactSuffix').html('');
                    }
                    if (mobileProviderNumberLimit) {
                        $('#newMobileFreeAlertContactNumberLimit').text(mobileProviderNumberLimit);
                    } else {
                        $('#newMobileFreeAlertContactNumberLimit').text('');
                    }
                    if (mobileProviderNumberLimitUp) {
                        $('#newMobileFreeAlertContactNumberLimitUp').text(mobileProviderNumberLimitUp);
                    } else {
                        $('#newMobileFreeAlertContactNumberLimitUp').text('');
                    }
                }
            });
        }
    ...
    var newAlertContactFormV = jQuery("#newAlertContactForm").validate({
        ...
        submitHandler: function(form) {
            jQuery(form).ajaxSubmit({
                success: function(theResponse) {
                    ...
                    $("#newAlertContactExistsErrorNotification").slideUp();
                    $("#newTwitterAlertContactNoFollowNotification").slideUp();
                    if ((theResponse).indexOf("true") > 0) {
                        $("#newAlertContactAttachNotification").slideDown("slow");
                        if ($("#newAlertContactType").val() == 1) {
                            $("#newAlertContactSuccessNotification").html('<strong class="alert-success">Alert contact created (activation required)!</strong> An activation SMS is sent. You can keep adding new alert contacts.');
                        }
                        ...
                        if ($("#newAlertContactType").val() == 3) {
                            $("#newAlertContactSuccessNotification").html('<strong class="alert-success">Alert contact created (activation required)!</strong> An activation direct message is sent. You can keep adding new alert contacts.');
                        }
                        if ($("#newAlertContactType").val() == 4 || $("#newAlertContactType").val() == 5 || $("#newAlertContactType").val() == 6 || $("#newAlertContactType").val() == 7 || $("#newAlertContactType").val() == 9 || $("#newAlertContactType").val() == 10 || $("#newAlertContactType").val() == 11) {
                            $("#newAlertContactSuccessNotification").html('<strong class="alert-success">Alert contact created!</strong> You can keep adding new alert contacts.');
                        }
                        if ($("#newAlertContactType").val() == 8) {
                            $("#newAlertContactSuccessNotification").html('<strong class="alert-success">Alert contact created (activation required)!</strong> An activation SMS is sent. You can keep adding new alert contacts.');
                        }
                        if ($("#newAlertContactType").val() == 14) {
                            $("#newAlertContactSuccessNotification").html('<strong class="alert-success">Alert contact created (activation required)!</strong> A voice call is now being made for delivering the activation code. You can keep adding new alert contacts.');
                        }
                        if ($("#newAlertContactType").val() == 18) {
                            var defaultMsg = '<strong class="alert-success">Alert contact created!</strong> You can keep adding new alert contacts.<br>';
                            var tr = $(theResponse);
                            var dacid = tr.attr("data-alertcontactid");
                            defaultMsg += 'But first, <a href="https://telegram.me/officialuptimerobot?start=' + dacid + '">click this custom link and start a dialog with UptimeRobot Bot</a>, to complete the activation of the <b>Telegram</b> alert contact.';
                            $("#newAlertContactSuccessNotification").html(defaultMsg);
                        }
                        if ($("#newAlertContactType").val() == 19) {
                            var defaultMsg = '<strong class="alert-success">Alert contact created!</strong> You can keep adding new alert contacts.<br>';
                            var tr = $(theResponse);
                            var dacid = tr.attr("data-alertcontactid");
                            defaultMsg += 'But first, please <em>sign in with Google</em> to complete Google AdWords Authentication process. After Google AdWords authentication you have to edit alert contact to select which campaigns should be managed depending on your monitor.<br><a class="sign_in_with_google" href="inc/lib/GoogleAdWords/index.php?auth=1&dacid=' + dacid + '" onclick="javascript:window.open(\'inc/lib/GoogleAdWords/index.php?auth=1&dacid=' + dacid + '\',\'google_auth_window\',\'width=480,height=550,toolbar=0,menubar=0,location=0,status=1,scrollbars=1,resizable=1,left=0,top=0\');return false;"><b>Sign in with Google</b></a>';
                            $("#newAlertContactSuccessNotification").html(defaultMsg);
                        }
                        if ($("#newAlertContactType").val() == 22) {
                            var defaultMsg = '<strong class="alert-success">Alert contact created!</strong> You can keep adding new alert contacts.<br>';
                            var tr = $(theResponse);
                            var dacid = tr.attr("data-alertcontactid");
                            defaultMsg += 'But first, please <em>sign in with Facebook</em> to complete Facebook Ads Authentication process. After Facebook Ads authentication you have to edit alert contact to select which campaigns should be managed depending on your monitor.<br><a class="sign_in_with_facebook" href="inc/lib/FacebookAds/index.php?auth=1&dacid=' + dacid + '" onclick="javascript:window.open(\'inc/lib/FacebookAds/index.php?auth=1&dacid=' + dacid + '\',\'facebook_auth_window\',\'width=480,height=550,toolbar=0,menubar=0,location=0,status=1,scrollbars=1,resizable=1,left=0,top=0\');return false;"><b>Sign in with Facebook</b></a>';
                            $("#newAlertContactSuccessNotification").html(defaultMsg);
                        }
                        newAlertContactFormV.resetForm();
                        $("#newAlertContactType").val("0").change();
                        $("#newMobileFreeProviderID").val("0").change();
                        $("#newProVoiceCallAlertContactCountry").val("0").change();
                        $("#newAlertContactSuccessNotification").slideDown("slow");
                        $(".alertContactsLimitList").val(parseInt($(".alertContactsLimitList").val()) + 1);
                        $('.alertContactsTableList > tbody').append(theResponse);
                    ...
                    if (theResponse == "PushbulletWrongAPIKey") {
                        $("#newAlertContactErrorNotification").slideDown("slow").html('The Pushbullet Access Token is not valid. Please check and try again.');
                    }
                    if (theResponse == "PushoverWrongAPIKey") {
                        $("#newAlertContactErrorNotification").slideDown("slow").html('The Pushover User Key is not valid. Please check and try again.');
                    }
                    if (theResponse == "alertContactExists") {
                        $("#newAlertContactExistsErrorNotification").slideDown("slow");
                    }
                    if (theResponse == "twitterNoFollow") {
                        $("#newTwitterAlertContactNoFollowNotification").slideDown("slow");
                    }
                    if (theResponse == "HipchatWrongAPIKey") {
                        $("#newAlertContactErrorNotification").slideDown("slow").html('The Hipchat Room ID or Token is not valid. Please check and try again.');
                    }
                    if (theResponse == "HipchatBrokenService") {
                        $("#newAlertContactErrorNotification").slideDown("slow").html("Couldn't communicate with the HipChat API. Please refresh the current page and try again.");
                    }
                    if (theResponse == "SlackError") {
                        $("#newAlertContactErrorNotification").slideDown("slow").html('The Slack WebHook URL is not valid. Please check and try again.');
                    }
                    if (theResponse == "MSTeamsBadRequest") {
                        $("#newAlertContactErrorNotification").slideDown("slow").html('Could not reach MS Teams servers. Please check and try again.');
                    }
                    if (theResponse == "HangoutsBadRequest") {
                        $("#newAlertContactErrorNotification").slideDown("slow").html('Given Google Hangouts Chat Room url is not valid. Please check and try again.');
                    }
                    if (theResponse == "DiscordError") {
                        $("#newAlertContactErrorNotification").slideDown("slow").html('Discord WebHook URL is not valid. Please check and try again.');
                    }
                },
                error: function(label) {
                    if (loader !== undefined) {
        ...
        rules: {
            ...
            newProSMSAlertContactFriendlyName: {
                required: "#newAlertContactType[value=8]",
                maxlength: 30
            },
            newProSMSAlertContactCountry: {
                required: "#newAlertContactType[value=8]",
                min: {
                    depends: "#newAlertContactType[value=8]",
                    param: 1
                },
            },
            newProSMSAlertContactValue: {
                required: "#newAlertContactType[value=8]",
                number: "#newAlertContactType[value=8]",
                minlength: 6,
                maxlength: 12,
                noZeroPrefix: "#newAlertContactType[value=8]"
            },
            newProSMSAlertContactExcludeNotifications: {
                required: "#newAlertContactType[value=8]",
                max: {
                    depends: "#newAlertContactType[value=8]",
                    param: function() {
                        if ($('#newProSMSAlertContactSSLExpirationReminder').is(":checked")) {
                            return 3
                        } else {
                            return 2
                        }
            ...
            newProSMSAlertContactSSLExpirationReminder: {
                required: "#newProSMSAlertContactExcludeNotifications[value=3]"
            },
            newMobileFreeAlertContactFriendlyName: {
                required: "#newAlertContactType[value=1]",
                maxlength: 30
            },
            newMobileFreeProviderID: {
                required: "#newAlertContactType[value=1]",
                min: {
                    depends: "#newAlertContactType[value=1]",
                    param: 1
                },
            },
            newMobileFreeAlertContactValue: {
                required: "#newAlertContactType[value=1]",
                minlength: {
                    depends: "#newAlertContactType[value=1]",
                    param: function() {
                        return parseInt($('#newMobileFreeAlertContactNumberLimit').text())
                    }
                },
                maxlength: {
                    depends: "#newAlertContactType[value=1]",
                    param: function() {
                        return parseInt($('#newMobileFreeAlertContactNumberLimitUp').text())
                    }
                },
            },
            newMobileFreeAlertContactExcludeNotifications: {
                required: "#newAlertContactType[value=1]",
                max: {
                    depends: "#newAlertContactType[value=1]",
                    param: function() {
                        if ($('#newMobileFreeAlertContactSSLExpirationReminder').is(":checked")) {
                            return 3
                        } else {
                            return 2
                        }
            ...
            newMobileFreeAlertContactSSLExpirationReminder: {
                required: "#newMobileFreeAlertContactExcludeNotifications[value=3]"
            },
            newEmailAlertContactFriendlyName: {
                required: "#newAlertContactType[value=2]",
                maxlength: 30
            },
            newEmailAlertContactValue: {
                required: "#newAlertContactType[value=2]",
                email: "#newAlertContactType[value=2]"
            },
            newEmailAlertContactExcludeNotifications: {
                required: "#newAlertContactType[value=2]",
                max: {
                    depends: "#newAlertContactType[value=2]",
                    param: function() {
                        if ($('#newEmailAlertContactSSLExpirationReminder').is(":checked")) {
                            return 3
                        } else {
                            return 2
                        }
            ...
            newEmailAlertContactSSLExpirationReminder: {
                required: "#newEmailAlertContactExcludeNotifications[value=3]"
            },
            newTwitterAlertContactFriendlyName: {
                required: "#newAlertContactType[value=3]",
                maxlength: 30
            },
            newTwitterAlertContactValue: {
                required: "#newAlertContactType[value=3]"
            },
            newTwitterAlertContactExcludeNotifications: {
                required: "#newAlertContactType[value=3]",
                max: {
                    depends: "#newAlertContactType[value=3]",
                    param: function() {
                        if ($('#newTwitterAlertContactSSLExpirationReminder').is(":checked")) {
                            return 3
                        } else {
                            return 2
                        }
            ...
            newTwitterAlertContactSSLExpirationReminder: {
                required: "#newTwitterAlertContactExcludeNotifications[value=3]"
            },
            newBoxcarAlertContactFriendlyName: {
                required: "#newAlertContactType[value=4]",
                maxlength: 30
            },
            newBoxcarAlertContactValue: {
                required: "#newAlertContactType[value=4]"
            },
            newBoxcarAlertContactExcludeNotifications: {
                required: "#newAlertContactType[value=4]",
                max: {
                    depends: "#newAlertContactType[value=4]",
                    param: function() {
                        if ($('#newBoxcarAlertContactSSLExpirationReminder').is(":checked")) {
                            return 3
                        } else {
                            return 2
                        }
            ...
            newBoxcarAlertContactSSLExpirationReminder: {
                required: "#newBoxcarAlertContactExcludeNotifications[value=3]"
            },
            newWebHookAlertContactFriendlyName: {
                required: "#newAlertContactType[value=5]",
                maxlength: 30
            },
            newWebHookAlertContactValue: {
                required: "#newAlertContactType[value=5]",
                url: "#newAlertContactType[value=5]",
                rangelength: [10, 1500]
            },
            newWebHookAlertContactPOSTValue: {
                checkJSON: "#newAlertContactType[value=5]",
                maxlength: 1300,
                required: function(element) {
                    if ($("#newWebHookAlertContactPOSTValueSendJSON").is(':checked')) {
            ...
            newWebHookAlertContactExcludeNotifications: {
                required: "#newAlertContactType[value=5]",
                max: {
                    depends: "#newAlertContactType[value=5]",
                    param: function() {
                        if ($('#newWebHookAlertContactSSLExpirationReminder').is(":checked")) {
                            return 3
                        } else {
                            return 2
                        }
            ...
            newWebHookAlertContactSSLExpirationReminder: {
                required: "#newWebHookAlertContactExcludeNotifications[value=3]"
            },
            newPushbulletAlertContactFriendlyName: {
                required: "#newAlertContactType[value=6]",
                maxlength: 30
            },
            newPushbulletAlertContactValue: {
                required: "#newAlertContactType[value=6]"
            },
            newPushbulletAlertContactExcludeNotifications: {
                required: "#newAlertContactType[value=6]",
                max: {
                    depends: "#newAlertContactType[value=6]",
                    param: function() {
                        if ($('#newPushbulletAlertContactSSLExpirationReminder').is(":checked")) {
                            return 3
                        } else {
                            return 2
                        }
            ...
            newPushbulletAlertContactSSLExpirationReminder: {
                required: "#newPushbulletAlertContactExcludeNotifications[value=3]"
            },
            newZapierAlertContactFriendlyName: {
                required: "#newAlertContactType[value=7]",
                maxlength: 30
            },
            newZapierAlertContactValue: {
                required: "#newAlertContactType[value=7]",
                url: "#newAlertContactType[value=7]"
            },
            newZapierAlertContactExcludeNotifications: {
                required: "#newAlertContactType[value=7]",
                max: {
                    depends: "#newAlertContactType[value=7]",
                    param: function() {
                        if ($('#newZapierAlertContactSSLExpirationReminder').is(":checked")) {
                            return 3
                        } else {
                            return 2
                        }
            ...
            newZapierAlertContactSSLExpirationReminder: {
                required: "#newZapierAlertContactExcludeNotifications[value=3]"
            },
            newPushoverAlertContactFriendlyName: {
                required: "#newAlertContactType[value=9]",
                maxlength: 30
            },
            newPushoverAlertContactValue: {
                required: "#newAlertContactType[value=9]"
            },
            newPushoverAlertContactExcludeNotifications: {
                required: "#newAlertContactType[value=9]",
                max: {
                    depends: "#newAlertContactType[value=9]",
                    param: function() {
                        if ($('#newPushoverAlertContactSSLExpirationReminder').is(":checked")) {
                            return 3
                        } else {
                            return 2
                        }
            ...
            newPushoverAlertContactSSLExpirationReminder: {
                required: "#newPushoverAlertContactExcludeNotifications[value=3]"
            },
            newHipchatAlertContactFriendlyName: {
                required: "#newAlertContactType[value=10]",
                maxlength: 30
            },
            newHipchatAlertContactValue: {
                required: "#newAlertContactType[value=10]",
                digits: "#newAlertContactType[value=10]"
            },
            newHipchatAlertContactToken: {
                required: "#newAlertContactType[value=10]"
            },
            newHipchatAlertContactServerURL: {
                url: "#newAlertContactType[value=10]"
            },
            newHipchatAlertContactExcludeNotifications: {
                required: "#newAlertContactType[value=10]",
                max: {
                    depends: "#newAlertContactType[value=10]",
                    param: function() {
                        if ($('#newHipchatAlertContactSSLExpirationReminder').is(":checked")) {
                            return 3
                        } else {
                            return 2
                        }
            ...
            newHipchatAlertContactSSLExpirationReminder: {
                required: "#newHipchatAlertContactExcludeNotifications[value=3]"
            },
            newSlackAlertContactFriendlyName: {
                required: "#newAlertContactType[value=11]",
                maxlength: 30
            },
            newSlackAlertContactValue: {
                required: "#newAlertContactType[value=11]",
                url: "#newAlertContactType[value=11]"
            },
            newSlackAlertContactExcludeNotifications: {
                required: "#newAlertContactType[value=11]",
                max: {
                    depends: "#newAlertContactType[value=11]",
                    param: function() {
                        if ($('#newSlackAlertContactSSLExpirationReminder').is(":checked")) {
                            return 3
                        } else {
                            return 2
                        }
            ...
            newSlackAlertContactSSLExpirationReminder: {
                required: "#newSlackAlertContactExcludeNotifications[value=3]"
            },
            newSplunkAlertContactFriendlyName: {
                required: "#newAlertContactType[value=15]",
                maxlength: 30
            },
            newSplunkAlertContactValue: {
                required: "#newAlertContactType[value=15]",
                url: "#newAlertContactType[value=15]"
            },
            newSplunkAlertContactExcludeNotifications: {
                required: "#newAlertContactType[value=15]",
                max: {
                    depends: "#newAlertContactType[value=15]",
                    param: function() {
                        if ($('#newSplunkAlertContactSSLExpirationReminder').is(":checked")) {
                            return 3
                        } else {
                            return 2
                        }
            ...
            newSplunkAlertContactSSLExpirationReminder: {
                required: "#newSplunkAlertContactExcludeNotifications[value=3]"
            },
            newPagerDutyAlertContactFriendlyName: {
                required: "#newAlertContactType[value=16]",
                maxlength: 30
            },
            newPagerDutyAlertContactValue: {
                required: "#newAlertContactType[value=16]",
            },
            newPagerDutyAlertContactExcludeNotifications: {
                required: "#newAlertContactType[value=16]",
                max: {
                    depends: "#newAlertContactType[value=16]",
                    param: function() {
                        if ($('#newPagerDutyAlertContactSSLExpirationReminder').is(":checked")) {
                            return 3
                        } else {
                            return 2
                        }
            ...
            newPagerDutyAlertContactSSLExpirationReminder: {
                required: "#newPagerDutyAlertContactExcludeNotifications[value=3]"
            },
            newOpsGenieAlertContactFriendlyName: {
                required: "#newAlertContactType[value=17]",
                maxlength: 30
            },
            newOpsGenieAlertContactValue: {
                required: "#newAlertContactType[value=17]",
                url: "#newAlertContactType[value=17]"
            },
            newOpsGenieAlertContactExcludeNotifications: {
                required: "#newAlertContactType[value=17]",
                max: {
                    depends: "#newAlertContactType[value=17]",
                    param: function() {
                        if ($('#newOpsGenieAlertContactSSLExpirationReminder').is(":checked")) {
                            return 3
                        } else {
                            return 2
                        }
            ...
            newOpsGenieAlertContactSSLExpirationReminder: {
                required: "#newOpsGenieAlertContactExcludeNotifications[value=3]"
            },
            newTelegramAlertContactFriendlyName: {
                required: "#newAlertContactType[value=18]",
                maxlength: 30
            },
            newTelegramAlertContactExcludeNotifications: {
                required: "#newAlertContactType[value=18]",
                max: {
                    depends: "#newAlertContactType[value=18]",
                    param: function() {
                        if ($('#newTelegramAlertContactSSLExpirationReminder').is(":checked")) {
                            return 3
                        } else {
                            return 2
                        }
            ...
            newTelegramAlertContactSSLExpirationReminder: {
                required: "#newTelegramAlertContactExcludeNotifications[value=3]"
            },
            newGoogleAdWordsAlertContactFriendlyName: {
                required: "#newAlertContactType[value=19]",
                maxlength: 30
            },
            newMSTeamsAlertContactFriendlyName: {
                required: "#newAlertContactType[value=20]",
                maxlength: 30
            },
            newMSTeamsAlertContactWebHookURL: {
                required: "#newAlertContactType[value=20]",
            },
            newMSTeamsAlertContactExcludeNotifications: {
                required: "#newAlertContactType[value=20]",
                max: {
                    depends: "#newAlertContactType[value=20]",
                    param: function() {
                        if ($('#newMSTeamsAlertContactSSLExpirationReminder').is(":checked")) {
                            return 3
                        } else {
                            return 2
                        }
            ...
            newMSTeamsAlertContactSSLExpirationReminder: {
                required: "#newMSTeamsAlertContactExcludeNotifications[value=3]"
            },
            newHangoutsChatAlertContactFriendlyName: {
                required: "#newAlertContactType[value=21]",
                maxlength: 30
            },
            newHangoutsChatAlertContactRoomURL: {
                required: "#newAlertContactType[value=21]",
                url: "#newAlertContactType[value=21]"
            },
            newHangoutsChatAlertContactExcludeNotifications: {
                required: "#newAlertContactType[value=21]",
                max: {
                    depends: "#newAlertContactType[value=21]",
                    param: function() {
                        if ($('#newHangoutsChatAlertContactSSLExpirationReminder').is(":checked")) {
                            return 3
                        } else {
                            return 2
                        }
            ...
            newHangoutsChatAlertContactSSLExpirationReminder: {
                required: "#newHangoutsChatAlertContactExcludeNotifications[value=3]"
            },
            newFacebookAdsAlertContactFriendlyName: {
                required: "#newAlertContactType[value=22]",
                maxlength: 30
            },
            newDiscordAlertContactFriendlyName: {
                required: "#newAlertContactType[value=23]",
                maxlength: 30
            },
            newDiscordAlertContactValue: {
                required: "#newAlertContactType[value=23]",
                url: "#newAlertContactType[value=23]"
            },
            newDiscordAlertContactExcludeNotifications: {
                required: "#newAlertContactType[value=23]",
                max: {
                    depends: "#newAlertContactType[value=23]",
                    param: function() {
                        if ($('#newDiscordAlertContactSSLExpirationReminder').is(":checked")) {
                            return 3
                        } else {
                            return 2
                        }
            ...
            newDiscordAlertContactSSLExpirationReminder: {
                required: "#newDiscordAlertContactExcludeNotifications[value=3]"
            },
        },
        messages: {
            newAlertContactType: {
                required: "Alert contact type is required",
                min: "Select the alert contact type"
            },
            newProSMSAlertContactFriendlyName: {
                required: "Friendly name is required",
            ...
            newProSMSAlertContactCountry: {
                min: "Select the mobile provider country"
            },
            newProSMSAlertContactValue: {
                required: "Mobile number is required",
                number: "Mobile number should be a number",
                minlength: "Mobile number should be min 6 digits",
                maxlength: "Mobile number should be max 11 digits",
                noZeroPrefix: "Mobile number must not start with 0"
            },
            newProSMSAlertContactExcludeNotifications: {
            ...
            newMobileFreeProviderID: {
                min: "Select the mobile provider"
            },
            newMobileFreeAlertContactValue: {
                required: "Mobile number is required",
                minlength: function() {
                    return 'Min. ' + $('#newMobileFreeAlertContactNumberLimit').text() + ' digits'
                },
                maxlength: function() {
                    return 'Max. ' + $('#newMobileFreeAlertContactNumberLimitUp').text() + ' digits'
                }
            },
            newMobileFreeAlertContactExcludeNotifications: {
                required: "Events to be received is required",
            ...
            newBoxcarAlertContactValue: {
                required: "Boxcar access token is required"
            },
            newBoxcarAlertContactExcludeNotifications: {
                required: "Events to be received is required",
                max: "Event or SSL notifications must be selected"
            ...
            newWebHookAlertContactValue: {
                required: "Web hook URL is required",
                url: "Please enter a valid URL"
            },
            newWebHookAlertContactPOSTValue: {
                checkJSON: "POST value must be JSON",
                required: "This field can not be empty when you select 'Send as JSON' option"
            },
            newWebHookAlertContactExcludeNotifications: {
                required: "Events to be received is required",
                max: "Event or SSL notifications must be selected"
            ...
            newZapierAlertContactValue: {
                required: "Zapier Hook URL is required",
                url: "Please enter a valid URL"
            },
            newZapierAlertContactExcludeNotifications: {
                required: "Events to be received is required",
            ...
            newHipchatAlertContactValue: {
                ...
                digits: "Room ID must be an integer"
            },
            newHipchatAlertContactToken: {
                required: "Room Token is required"
            },
            newHipchatAlertContactServerURL: {
                url: "Server URL is not valid"
            },
            newHipchatAlertContactExcludeNotifications: {
                required: "Events to be received is required",
                max: "Event or SSL notifications must be selected"
            ...
            newSplunkAlertContactValue: {
                required: "Splunk URL is required",
                url: "Splunk URL is not valid"
            },
            newSplunkAlertContactExcludeNotifications: {
                required: "Events to be received is required",
                max: "Event or SSL notifications must be selected"
            ...
            newPagerDutyAlertContactValue: {
                required: "PagerDuty URL is required",
                url: "PagerDuty URL is not valid"
            },
            newPagerDutyAlertContactExcludeNotifications: {
                required: "Events to be received is required",
                max: "Event or SSL notifications must be selected"
            ...
            newOpsGenieAlertContactValue: {
                required: "OpsGenie URL is required",
                url: "OpsGenie URL is not valid"
            },
            newOpsGenieAlertContactExcludeNotifications: {
                required: "Events to be received is required",
                max: "Event or SSL notifications must be selected"
            ...
            newHangoutsChatAlertContactRoomURL: {
                required: "Room URL is required",
                url: "Room URL is not valid"
            },
            newHangoutsChatAlertContactExcludeNotifications: {
                required: "Events to be received is required",
                max: "Event or SSL notifications must be selected"
    ...
    $(document).on("click", '.deleteAlertContact', function(e) {
        ...
        $('#deleteAlertContactForm').attr('action', 'inc/dml/userDML.php?action=deleteAlertContact&alertContactID=' + alertContactID);
    ...
    $('.newWebHookAlertContactDetailsButton').click(function(e) {
        e.preventDefault();
        $('.newWebHookAlertContactDetails').slideToggle('slow');
    });
    $(document).on("click", '.editAlertContact', function(e) {
        ...
        var title = $(this).closest('td').prev('td').prev('td').html();
        var alertContactType = $(title).attr("title");
        $('#editMobileFreeAlertContact').addClass('hide');
        $('#editEmailAlertContact').addClass('hide');
        $('#editTwitterAlertContact').addClass('hide');
        $('#editBoxcarAlertContact').addClass('hide');
        $('#editWebHookAlertContact').addClass('hide');
        $('#editPushbulletAlertContact').addClass('hide');
        $('#editZapierAlertContact').addClass('hide');
        $('#editProSMSAlertContact').addClass('hide');
        $('#editPushoverAlertContact').addClass('hide');
        $('#editHipchatAlertContact').addClass('hide');
        $('#editSlackAlertContact').addClass('hide');
        $('#editSplunkAlertContact').addClass('hide');
        $('#editPagerDutyAlertContact').addClass('hide');
        $('#editOpsGenieAlertContact').addClass('hide');
        $('#editTelegramAlertContact').addClass('hide');
        $('#editGoogleAdWordsAlertContact').addClass('hide');
        $('#editFacebookAdsAlertContact').addClass('hide');
        $('#editProVoiceCallAlertContact').addClass('hide');
        $('#editIosAppAlertContact').addClass('hide');
        $('#editAndroidAppAlertContact').addClass('hide');
        $('#editBaseAccountAlertContact').addClass('hide');
        $("#editMSTeamsAlertContact").addClass("hide");
        $("#editHangoutsChatAlertContact").addClass("hide");
        $('#editDiscordAlertContact').addClass('hide');
        $('#editAlertContactForm').attr('action', 'inc/dml/userDML.php?action=editAlertContact&alertContactID=' + alertContactID);
        $("#editAlertContactSuccessNotification").slideUp();
        $("#editAlertContactNoGoogleAdwordsCampaignNotification").slideUp();
        $("#editAlertContactNoFacebookAdsCampaignNotification").slideUp();
        $("#editAlertContactExistsNotification").slideUp();
        $('#editHipchatAlertContactCustomValue').val("");
        $('#editSlackAlertContactCustomValue').val("");
        $('#editSplunkAlertContactValue').val("");
        $('#editPagerDutyAlertContactValue').val("");
        $('#editOpsGenieAlertContactValue').val("");
        $('#editDiscordAlertContactCustomValue').val('');
        $("#editProVoiceCallAlertContactValue").val("");
        $("#editProVoiceCallAlertContactFriendlyName").val("");
        $("#editMobileFreeAlertContactSSLExpirationReminder").removeAttr("checked");
        $("#editEmailAlertContactSSLExpirationReminder").removeAttr("checked");
        $("#editTwitterAlertContactSSLExpirationReminder").removeAttr("checked");
        $("#editBoxcarAlertContactSSLExpirationReminder").removeAttr("checked");
        $("#editWebHookAlertContactSSLExpirationReminder").removeAttr("checked");
        $("#editPushbulletAlertContactSSLExpirationReminder").removeAttr("checked");
        $("#editZapierAlertContactSSLExpirationReminder").removeAttr("checked");
        $("#editProSMSAlertContactSSLExpirationReminder").removeAttr("checked");
        $("#editProVoiceCallAlertContactSSLExpirationReminder").removeAttr("checked");
        $("#editPushoverAlertContactSSLExpirationReminder").removeAttr("checked");
        $("#editHipchatAlertContactSSLExpirationReminder").removeAttr("checked");
        $("#editSlackAlertContactSSLExpirationReminder").removeAttr("checked");
        $("#editSplunkAlertContactSSLExpirationReminder").removeAttr("checked");
        $("#editPagerDutyAlertContactSSLExpirationReminder").removeAttr("checked");
        $("#editOpsGenieAlertContactSSLExpirationReminder").removeAttr("checked");
        $("#editTelegramAlertContactSSLExpirationReminder").removeAttr("checked");
        $("#editIosAppAlertContactSSLExpirationReminder").removeAttr("checked");
        $("#editAndroidAppAlertContactSSLExpirationReminder").removeAttr("checked");
        $("#editBaseAccountAlertContactSSLExpirationReminder").removeAttr("checked");
        $("#editMSTeamsAlertContactSSLExpirationReminder").removeAttr("checked");
        $("#editHangoutsChatAlertContactSSLExpirationReminder").removeAttr("checked");
        $("#editDiscordAlertContactSSLExpirationReminder").removeAttr("checked");
        $('#editGoogleAdWordsAlertContactCustomValue2').val("");
        $('#editGoogleAdWordsAlertContactValue').find("option").remove();
        $('#editGoogleAdWordsAlertContactValue').select2("val", "");
        $('#editGoogleAdWordsAlertContactCustomValue').find("option").remove();
        $('#editGoogleAdWordsAlertContactCustomValue').select2("val", "");
        $('#editSelectedGoogleAdWordsAlertContactCustomValue').select2("val", "");
        $('#editFacebookAdsAlertContactCustomValue').val("");
        $('#editFacebookAdsAlertContactCustomValue').find("option").remove();
        $('#editFacebookAdsAlertContactValue').select2("val", "");
        $('#editFacebookAdsAlertContactValue').find("option").remove();
        $('#editPagerDutyAlertContactAutoResolve').select2("val", "");
        $('#editOpsGenieAlertContactTeamsToNotify').val("");
        if (alertContactType == "Google AdWords") {
            $('#editAlertContactActivationCodeContainer').addClass("hide");
            $('#editAlertContact #editAlertContactFormSubmitButton').addClass("hide");
            $('#editGoogleAdWordsAlertNoAuth').addClass("hide");
            $('#editGoogleAdWordsAlertYesAuth').addClass("hide");
            $.ajax({
                ...
                success: function(theResponse) {
                    if (theResponse == 1) {
                        ...
                        var a = $('#editGoogleAdWordsAlertNoAuth').find("a");
                        var f = a.closest("form#editAlertContactForm");
                        f.attr("data-alertcontactid", alertContactID);
                        var ahref = a.attr("href");
                        ahref = ahref.split("dacid=");
                        var nhref = ahref[0] + "dacid=" + alertContactID;
                        a.attr("href", nhref);
                        a.attr("onclick", "javascript:window.open('" + nhref + "','google_auth_window','width=480,height=550,toolbar=0,menubar=0,location=0,status=1,scrollbars=1,resizable=1,left=0,top=0');return false;");
                        $('#editGoogleAdWordsAlertNoAuth').removeClass("hide");
                    }
                }
            });
        } else if (alertContactType == "Facebook Ads") {
            ...
            $.ajax({
                ...
                success: function(theResponse) {
                    if (theResponse == 1) {
                        $('#editFacebookAdsAlertYesAuth').removeClass("hide");
                    } else {
                        var a = $('#editFacebookAdsAlertNoAuth').find("a");
                        var f = a.closest("form#editAlertContactForm");
                        f.attr("data-alertcontactid", alertContactID);
                        var ahref = a.attr("href");
                        ahref = ahref.split("dacid=");
                        ...
                        a.attr("href", nhref);
                        a.attr("onclick", "javascript:window.open('" + nhref + "','facebook_auth_window','width=480,height=550,toolbar=0,menubar=0,location=0,status=1,scrollbars=1,resizable=1,left=0,top=0');return false;");
                        $('#editFacebookAdsAlertNoAuth').removeClass("hide");
                    }
                }
            });
        }
        $('#editMobileFreeAlertContactExcludeNotifications').select2("val", "");
        $('#editEmailAlertContactExcludeNotifications').select2("val", "");
        $('#editTwitterAlertContactExcludeNotifications').select2("val", "");
        $('#editBoxcarAlertContactExcludeNotifications').select2("val", "");
        $('#editWebHookAlertContactExcludeNotifications').select2("val", "");
        $('#editZapierAlertContactExcludeNotifications').select2("val", "");
        $('#editProSMSAlertContactExcludeNotifications').select2("val", "");
        $('#editProVoiceCallAlertContactExcludeNotifications').select2("val", "");
        $('#editPushoverAlertContactExcludeNotifications').select2("val", "");
        $('#editHipchatAlertContactExcludeNotifications').select2("val", "");
        $('#editSlackAlertContactExcludeNotifications').select2("val", "");
        $('#editSplunkAlertContactExcludeNotifications').select2("val", "");
        $('#editPagerDutyAlertContactExcludeNotifications').select2("val", "");
        $('#editOpsGenieAlertContactExcludeNotifications').select2("val", "");
        $('#editTelegramAlertContactExcludeNotifications').select2("val", "");
        $('#editIosAppAlertContactExcludeNotifications').select2("val", "");
        $('#editAndroidAppAlertContactExcludeNotifications').select2("val", "");
        $('#editBaseAccountAlertContactExcludeNotifications').select2("val", "");
        $('#editMSTeamsAlertContactExcludeNotifications').select2("val", "");
        $('#editHangoutsChatAlertContactExcludeNotifications').select2("val", "");
        ...
        $.ajax({
            ...
            success: function(theResponse) {
                ...
                if (alertContactType == 19) {
                    var alertContactValue = json[0]['alertContactValue'];
                    var alertContactFriendlyName = json[0]['alertContactFriendlyName'];
                    var alertContactCustomValue = json[0]['alertContactCustomValue'];
                    var alertContactCustomValue2 = json[0]['alertContactCustomValue2'];
                } else if (alertContactType == 22) {
                    var alertContactValue = json[0]['alertContactValue'];
                    var alertContactFriendlyName = json[0]['alertContactFriendlyName'];
                    var alertContactCustomValue = json[0]['alertContactCustomValue'];
                } else {
                    var alertContactValue = fRsi(json[0]['alertContactValue']);
                    var alertContactFriendlyName = fRsi(json[0]['alertContactFriendlyName']);
                    var alertContactCustomValue2 = fRsi(json[0]['alertContactCustomValue2']);
                    var alertContactCustomValue4 = fRsi(json[0]['alertContactCustomValue4']);
                    var alertContactCustomValue = fRsi(json[0]['alertContactCustomValue']);
                }
                var alertContactSSLExpirationReminder = json[0]['alertContactSSLExpirationReminder'];
                var alertContactExclude = json[0]['alertContactExclude'];
                if (alertContactType == 5 || alertContactType == 9 || alertContactType == 10) {
                    var alertContactCustomValue = jQuery.parseJSON(json[0]['alertContactCustomValue']);
                } else {
                    if (json[0]['alertContactCustomValue'] != null) {
                        if (alertContactType != 19 && alertContactType != 22) {
                            var alertContactCustomValue = fRsi(json[0]['alertContactCustomValue']);
                        }
                    } else {
                        var alertContactCustomValue = '';
                    }
                }
                if (alertContactType == 1) {
                    $('#editAlertContactTypeTitle').html('Free Mobile SMS');
                    $('#editAlertContactType').val(alertContactType);
                    $('#editMobileFreeAlertContact').removeClass('hide');
                    $('#editMobileFreeAlertContactFriendlyName').val(alertContactFriendlyName);
                    $('#editMobileFreeAlertContactValue').html(alertContactValue);
                    if (alertContactSSLExpirationReminder == 1 || alertContactSSLExpirationReminder == "1") {
                        $("#editMobileFreeAlertContactSSLExpirationReminder").attr("checked", "checked");
                    } else {
                        $("#editMobileFreeAlertContactSSLExpirationReminder").removeAttr("checked");
                    }
                    $('#editMobileFreeAlertContactExcludeNotifications').select2("val", alertContactExclude);
                }
                if (alertContactType == 2) {
                    ...
                    if (alertContactSSLExpirationReminder == 1 || alertContactSSLExpirationReminder == "1") {
                        $("#editEmailAlertContactSSLExpirationReminder").attr("checked", "checked");
                    } else {
                        $("#editEmailAlertContactSSLExpirationReminder").removeAttr("checked");
                    }
                    $('#editEmailAlertContactExcludeNotifications').select2("val", alertContactExclude);
                }
                if (alertContactType == 3) {
                    $('#editAlertContactTypeTitle').html('Twitter');
                    $('#editAlertContactType').val(alertContactType);
                    $('#editTwitterAlertContact').removeClass('hide');
                    $('#editTwitterAlertContactFriendlyName').val(alertContactFriendlyName);
                    $('#editTwitterAlertContactValue').html(alertContactValue);
                    if (alertContactSSLExpirationReminder == 1 || alertContactSSLExpirationReminder == "1") {
                        $("#editTwitterAlertContactSSLExpirationReminder").attr("checked", "checked");
                    } else {
                        $("#editTwitterAlertContactSSLExpirationReminder").removeAttr("checked");
                    }
                    $('#editTwitterAlertContactExcludeNotifications').select2("val", alertContactExclude);
                }
                if (alertContactType == 4) {
                    if (alertContactID <= 2255562) {
                        $('#editAlertContactTypeTitle').html('Boxcar');
                    } else {
                        $('#editAlertContactTypeTitle').html('Boxcar 2');
                    }
                    $('#editAlertContactType').val(alertContactType);
                    $('#editBoxcarAlertContact').removeClass('hide');
                    $('#editBoxcarAlertContactFriendlyName').val(alertContactFriendlyName);
                    $('#editBoxcarAlertContactValue').html(alertContactValue);
                    if (alertContactSSLExpirationReminder == 1 || alertContactSSLExpirationReminder == "1") {
                        $("#editBoxcarAlertContactSSLExpirationReminder").attr("checked", "checked");
                    } else {
                        $("#editBoxcarAlertContactSSLExpirationReminder").removeAttr("checked");
                    }
                    $('#editBoxcarAlertContactExcludeNotifications').select2("val", alertContactExclude);
                }
                if (alertContactType == 5) {
                    $('#editAlertContactTypeTitle').html('Web-Hook');
                    $('#editAlertContactType').val(alertContactType);
                    $('#editWebHookAlertContact').removeClass('hide');
                    $('#editWebHookAlertContactFriendlyName').val(alertContactFriendlyName);
                    $('#editWebHookAlertContactValue').val(alertContactValue);
                    if (alertContactCustomValue) {
                        $('#editWebHookAlertContactPOSTValue').val(fRsi(alertContactCustomValue.postValue));
                        if (alertContactCustomValue.sendJSON == 1) {
                            $('#editWebHookAlertContactPOSTValueSendJSON').prop("checked", true);
                        }
                    }
                    var sendQueryString = (Number(alertContactCustomValue4) == 1) ? true : false;
                    $("#editWebHookAlertContactValueSendQueryString").prop("checked", sendQueryString)
                    if (alertContactSSLExpirationReminder == 1 || alertContactSSLExpirationReminder == "1") {
                        $("#editWebHookAlertContactSSLExpirationReminder").attr("checked", "checked");
                    } else {
                        $("#editWebHookAlertContactSSLExpirationReminder").removeAttr("checked");
                    }
                    $('#editWebHookAlertContactExcludeNotifications').select2("val", alertContactExclude);
                }
                if (alertContactType == 6) {
                    $('#editAlertContactTypeTitle').html('Pushbullet');
                    $('#editAlertContactType').val(alertContactType);
                    $('#editPushbulletAlertContact').removeClass('hide');
                    $('#editPushbulletAlertContactFriendlyName').val(alertContactFriendlyName);
                    $('#editPushbulletAlertContactValue').html(alertContactValue);
                    if (alertContactSSLExpirationReminder == 1 || alertContactSSLExpirationReminder == "1") {
                        $("#editPushbulletAlertContactSSLExpirationReminder").attr("checked", "checked");
                    } else {
                        $("#editPushbulletAlertContactSSLExpirationReminder").removeAttr("checked");
                    }
                    $('#editPushbulletAlertContactExcludeNotifications').select2("val", alertContactExclude);
                }
                if (alertContactType == 7) {
                    $('#editAlertContactTypeTitle').html('Zapier');
                    $('#editAlertContactType').val(alertContactType);
                    $('#editZapierAlertContact').removeClass('hide');
                    $('#editZapierAlertContactFriendlyName').val(alertContactFriendlyName);
                    $('#editZapierAlertContactValue').val(alertContactValue);
                    if (alertContactSSLExpirationReminder == 1 || alertContactSSLExpirationReminder == "1") {
                        $("#editZapierAlertContactSSLExpirationReminder").attr("checked", "checked");
                    } else {
                        $("#editZapierAlertContactSSLExpirationReminder").removeAttr("checked");
                    }
                    $('#editZapierAlertContactExcludeNotifications').select2("val", alertContactExclude);
                }
                if (alertContactType == 8) {
                    $('#editAlertContactTypeTitle').html('Pro SMS');
                    $('#editAlertContactType').val(alertContactType);
                    $('#editProSMSAlertContact').removeClass('hide');
                    $('#editProSMSAlertContactFriendlyName').val(alertContactFriendlyName);
                    $('#editProSMSAlertContactValue').html(alertContactValue);
                    if (alertContactSSLExpirationReminder == 1 || alertContactSSLExpirationReminder == "1") {
                        $("#editProSMSAlertContactSSLExpirationReminder").attr("checked", "checked");
                    } else {
                        $("#editProSMSAlertContactSSLExpirationReminder").removeAttr("checked");
                    }
                    $('#editProSMSAlertContactExcludeNotifications').select2("val", alertContactExclude);
                }
                if (alertContactType == 9) {
                    $('#editAlertContactTypeTitle').html('Pushover');
                    $('#editAlertContactType').val(alertContactType);
                    $('#editPushoverAlertContact').removeClass('hide');
                    $('#editPushoverAlertContactFriendlyName').val(alertContactFriendlyName);
                    $('#editPushoverAlertContactValue').html(alertContactValue);
                    if (alertContactCustomValue) {
                        $('#editPushoverAlertContactPriority').select2("val", fRsi(alertContactCustomValue.priority));
                    } else {
                        $('#editPushoverAlertContactPriority').select2("val", 0);
                    }
                    if (alertContactSSLExpirationReminder == 1 || alertContactSSLExpirationReminder == "1") {
                        $("#editPushoverAlertContactSSLExpirationReminder").attr("checked", "checked");
                    } else {
                        $("#editPushoverAlertContactSSLExpirationReminder").removeAttr("checked");
                    }
                    $('#editPushoverAlertContactExcludeNotifications').select2("val", alertContactExclude);
                }
                if (alertContactType == 10) {
                    $('#editAlertContactTypeTitle').html('HipChat');
                    $('#editAlertContactType').val(alertContactType);
                    $('#editHipchatAlertContact').removeClass('hide');
                    $('#editHipchatAlertContactFriendlyName').val(alertContactFriendlyName);
                    $('#editHipchatAlertContactValue').html(alertContactValue);
                    $('#editHipchatAlertContactToken').html(fRsi(alertContactCustomValue.token));
                    $('#editHipchatAlertContactServerURL').html(fRsi(alertContactCustomValue.serverURL));
                    $('#editHipchatAlertContactCustomValue').val(fRsi(alertContactCustomValue2));
                    if (alertContactSSLExpirationReminder == 1 || alertContactSSLExpirationReminder == "1") {
                        $("#editHipchatAlertContactSSLExpirationReminder").attr("checked", "checked");
                    } else {
                        $("#editHipchatAlertContactSSLExpirationReminder").removeAttr("checked");
                    }
                    $('#editHipchatAlertContactExcludeNotifications').select2("val", alertContactExclude);
                }
                if (alertContactType == 11) {
                    $('#editAlertContactTypeTitle').html('Slack');
                    $('#editAlertContactType').val(alertContactType);
                    $('#editSlackAlertContact').removeClass('hide');
                    $('#editSlackAlertContactFriendlyName').val(alertContactFriendlyName);
                    $('#editSlackAlertContactValue').html(alertContactValue);
                    $('#editSlackAlertContactCustomValue').val(fRsi(alertContactCustomValue2));
                    $('#editSlackAlertContactCustomValue2').val(fRsi(alertContactCustomValue));
                    if (alertContactSSLExpirationReminder == 1 || alertContactSSLExpirationReminder == "1") {
                        $("#editSlackAlertContactSSLExpirationReminder").attr("checked", "checked");
                    } else {
                        $("#editSlackAlertContactSSLExpirationReminder").removeAttr("checked");
                    }
                    $('#editSlackAlertContactExcludeNotifications').select2("val", alertContactExclude);
                }
                if (alertContactType == 12) {
                    $('#editAlertContactTypeTitle').html('IOS');
                    $('#editAlertContactType').val(alertContactType);
                    $('#editIosAppAlertContact').removeClass('hide');
                    $('#editIosAppAlertContactFriendlyName').val(alertContactFriendlyName);
                    if (alertContactSSLExpirationReminder == 1 || alertContactSSLExpirationReminder == "1") {
                        $("#editIosAppAlertContactSSLExpirationReminder").attr("checked", "checked");
                    } else {
                        $("#editIosAppAlertContactSSLExpirationReminder").removeAttr("checked");
                    }
                    $('#editIosAppAlertContactExcludeNotifications').select2("val", alertContactExclude);
                }
                if (alertContactType == 13) {
                    $('#editAlertContactTypeTitle').html('Android');
                    $('#editAlertContactType').val(alertContactType);
                    $('#editAndroidAppAlertContact').removeClass('hide');
                    $('#editAndroidAppAlertContactFriendlyName').val(alertContactFriendlyName);
                    if (alertContactSSLExpirationReminder == 1 || alertContactSSLExpirationReminder == "1") {
                        $("#editAndroidAppAlertContactSSLExpirationReminder").attr("checked", "checked");
                    } else {
                        $("#editAndroidAppAlertContactSSLExpirationReminder").removeAttr("checked");
                    }
                    $('#editAndroidAppAlertContactExcludeNotifications').select2("val", alertContactExclude);
                }
                if (alertContactType == 14) {
                    $('#editAlertContactTypeTitle').html('Voice Call');
                    $('#editAlertContactType').val(alertContactType);
                    $('#editProVoiceCallAlertContact').removeClass('hide');
                    $('#editProVoiceCallAlertContactFriendlyName').val(alertContactFriendlyName);
                    $('#editProVoiceCallAlertContactValue').html(alertContactValue);
                    if (alertContactSSLExpirationReminder == 1 || alertContactSSLExpirationReminder == "1") {
                        $("#editProVoiceCallAlertContactSSLExpirationReminder").attr("checked", "checked");
                    } else {
                        $("#editProVoiceCallAlertContactSSLExpirationReminder").removeAttr("checked");
                    }
                    $('#editProVoiceCallAlertContactExcludeNotifications').select2("val", alertContactExclude);
                }
                if (alertContactType == 15) {
                    $('#editAlertContactTypeTitle').html('Splunk');
                    $('#editAlertContactType').val(alertContactType);
                    $('#editSplunkAlertContact').removeClass('hide');
                    $('#editSplunkAlertContactFriendlyName').val(alertContactFriendlyName);
                    $('#editSplunkAlertContactValue').val(alertContactValue);
                    if (alertContactSSLExpirationReminder == 1 || alertContactSSLExpirationReminder == "1") {
                        $("#editSplunkAlertContactSSLExpirationReminder").attr("checked", "checked");
                    } else {
                        $("#editSplunkAlertContactSSLExpirationReminder").removeAttr("checked");
                    }
                    $('#editSplunkAlertContactExcludeNotifications').select2("val", alertContactExclude);
                }
                if (alertContactType == 16) {
                    $('#editAlertContactTypeTitle').html('PagerDuty');
                    $('#editAlertContactType').val(alertContactType);
                    $('#editPagerDutyAlertContact').removeClass('hide');
                    $('#editPagerDutyAlertContactFriendlyName').val(alertContactFriendlyName);
                    $('#editPagerDutyAlertContactValue').val(alertContactValue);
                    $('#editPagerDutyAlertContactAutoResolve').select2("val", alertContactCustomValue);
                    if (alertContactSSLExpirationReminder == 1 || alertContactSSLExpirationReminder == "1") {
                        $("#editPagerDutyAlertContactSSLExpirationReminder").attr("checked", "checked");
                    } else {
                        $("#editPagerDutyAlertContactSSLExpirationReminder").removeAttr("checked");
                    }
                    $('#editPagerDutyAlertContactExcludeNotifications').select2("val", alertContactExclude);
                }
                if (alertContactType == 17) {
                    $('#editAlertContactTypeTitle').html('OpsGenie');
                    $('#editAlertContactType').val(alertContactType);
                    $('#editOpsGenieAlertContact').removeClass('hide');
                    $('#editOpsGenieAlertContactFriendlyName').val(alertContactFriendlyName);
                    $('#editOpsGenieAlertContactValue').val(alertContactValue);
                    $('#editOpsGenieAlertContactTeamsToNotify').val(alertContactCustomValue);
                    if (alertContactSSLExpirationReminder == 1 || alertContactSSLExpirationReminder == "1") {
                        $("#editOpsGenieAlertContactSSLExpirationReminder").attr("checked", "checked");
                    } else {
                        $("#editOpsGenieAlertContactSSLExpirationReminder").removeAttr("checked");
                    }
                    $('#editOpsGenieAlertContactExcludeNotifications').select2("val", alertContactExclude);
                }
                if (alertContactType == 18) {
                    $('#editAlertContactTypeTitle').html('Telegram');
                    $('#editAlertContactType').val(alertContactType);
                    $('#editTelegramAlertContact').removeClass('hide');
                    $('#editTelegramAlertContactFriendlyName').val(alertContactFriendlyName);
                    if ($.trim(alertContactValue) == "") {
                        var a = $('#editTelegramAlertNotStarted').find("a");
                        var ahref = a.attr("href");
                        ahref = ahref.split("=");
                        a.attr("href", ahref[0] + "=" + alertContactID);
                        $('#editTelegramAlertStarted').addClass("hide");
                        $('#editTelegramAlertNotStarted').removeClass('hide');
                    } else {
                        var deets = "";
                        if ($.trim(alertContactCustomValue) != "") {
                            deets = alertContactCustomValue + " [" + alertContactValue + "]";
                        } else {
                            deets = "Telegram Chat ID : " + alertContactValue;
                        }
                        var s = $('#editTelegramAlertStarted').find("span");
                        s.html(deets);
                        $('#editTelegramAlertNotStarted').addClass("hide");
                        $('#editTelegramAlertStarted').removeClass('hide');
                    }
                    if (alertContactSSLExpirationReminder == 1 || alertContactSSLExpirationReminder == "1") {
                        $("#editTelegramAlertContactSSLExpirationReminder").attr("checked", "checked");
                    } else {
                        $("#editTelegramAlertContactSSLExpirationReminder").removeAttr("checked");
                    }
                    $('#editTelegramAlertContactExcludeNotifications').select2("val", alertContactExclude);
                }
                if (alertContactType == 19) {
                    $('#editAlertContactTypeTitle').html('Google AdWords');
                    $('#editAlertContactType').val(alertContactType);
                    $('#editGoogleAdWordsAlertContact').removeClass('hide');
                    $('#editGoogleAdWordsAlertContactFriendlyName').val(alertContactFriendlyName);
                    $('#editGoogleAdWordsAlertContactCustomValue2').val(alertContactCustomValue2);
                    $("#editGoogleAdWordsAlertContactValue").data("data-acid", alertContactID);
                    $("#editGoogleAdWordsAlertContactCustomValue").data("data-acid", alertContactID);
                    if (alertContactCustomValue.length > 0) {
                        $.each(alertContactCustomValue, function(i, data) {
                            var newOption = new Option(data.text, data.id, true, true);
                            $('#editGoogleAdWordsAlertContactCustomValue').append(newOption).trigger('change');
                            $('#editGoogleAdWordsAlertContactCustomValue').val([data.id]).trigger('change');
                            $('#s2id_editGoogleAdWordsAlertContactCustomValue .select2-chosen').text(data.text);
                        });
                        $.each(alertContactValue, function(i, data) {
                            var newOption = new Option(data.text + " (" + data.state + ")", data.id, true, true);
                            $('#editSelectedGoogleAdWordsAlertContactValue').append(newOption).trigger('change');
                        });
                    }
                }
                if (alertContactType == 20) {
                    $('#editAlertContactTypeTitle').html('MS Teams');
                    $('#editAlertContactType').val(alertContactType);
                    $('#editMSTeamsAlertContact').removeClass('hide');
                    $('#editMSTeamsAlertContactFriendlyName').val(alertContactFriendlyName);
                    $('#editMSTeamsAlertContactWebHookURL').html(alertContactValue);
                    if (alertContactSSLExpirationReminder == 1 || alertContactSSLExpirationReminder == "1") {
                        $("#editMSTeamsAlertContactSSLExpirationReminder").attr("checked", "checked");
                    } else {
                        $("#editMSTeamsAlertContactSSLExpirationReminder").removeAttr("checked");
                    }
                    $('#editMSTeamsAlertContactExcludeNotifications').select2("val", alertContactExclude);
                }
                if (alertContactType == 21) {
                    $('#editAlertContactTypeTitle').html('Google Hangouts Chat');
                    $('#editAlertContactType').val(alertContactType);
                    $('#editHangoutsChatAlertContact').removeClass('hide');
                    $('#editHangoutsChatAlertContactFriendlyName').val(alertContactFriendlyName);
                    $('#editHangoutsChatAlertContactRoomURL').val(alertContactValue);
                    $('#editHangoutsChatAlertContactCustomValue').val(alertContactCustomValue);
                    if (alertContactSSLExpirationReminder == 1 || alertContactSSLExpirationReminder == "1") {
                        $("#editHangoutsChatAlertContactSSLExpirationReminder").attr("checked", "checked");
                    } else {
                        $("#editHangoutsChatAlertContactSSLExpirationReminder").removeAttr("checked");
                    }
                    $('#editHangoutsChatAlertContactExcludeNotifications').select2("val", alertContactExclude);
                }
                if (alertContactType == 22) {
                    $('#editFacebookAdsAlertContactValue').val("");
                    $('#editFacebookAdsAlertContactValue').find("option").remove();
                    $('#editFacebookAdsAlertContactValue').trigger("change");
                    $('#editFacebookAdsAlertContactCustomValue').val("");
                    $('#editFacebookAdsAlertContactCustomValue').find("option").remove();
                    $('#editFacebookAdsAlertContactCustomValue').trigger("change");
                    $('#editAlertContactTypeTitle').html('Facebook Ads');
                    $('#editAlertContactType').val(alertContactType);
                    $('#editFacebookAdsAlertContact').removeClass('hide');
                    $('#editFacebookAdsAlertContactFriendlyName').val(alertContactFriendlyName);
                    var alertContactValue = json[0]['alertContactValue'];
                    var alertContactCustomValue = json[0]['alertContactCustomValue'];
                    $("#editFacebookAdsUserName").html(alertContactCustomValue["facebookAdsUserName"]);
                    $("#editFacebookAdsUserId").html(alertContactCustomValue["facebookAdsUserId"]);
                    var facebookAdsAccounts = alertContactCustomValue["facebookAdsAccounts"];
                    var facebookAdsAccountsSelected = null;
                    if (facebookAdsAccounts.length > 0) {
                        $('#editFacebookAdsAlertContactCustomValue').on("change", function(e) {
                            var value = e.val;
                            if (value && value.indexOf("act_") > -1) {
                                setFacebookAdsAccountsSelectOptions(alertContactID, value);
                            } else {
                                $('#editFacebookAdsAlertContactValue').val("");
                                $('#editFacebookAdsAlertContactValue').find("option").remove();
                                $('#editFacebookAdsAlertContactValue').trigger("change");
                                $("#facebookAdsApiReachError").remove();
                            }
                        });
                        $('#editFacebookAdsAlertContactValue').on("change", function(e) {
                            $("#editFacebookAdsAlertContactValueArray").val($('#editFacebookAdsAlertContactValue').val());
                        });
                        $.each(facebookAdsAccounts, function(i, data) {
                            var newOption = new Option(data.name + ' (' + data.id + ')', data.id, false, false);
                            $('#editFacebookAdsAlertContactCustomValue').append(newOption);
                            if (data.id == alertContactCustomValue["facebookAdsAccountsSelected"]) {
                                $('#editFacebookAdsAlertContactCustomValue').val([data.id]).trigger('change');
                                $('#s2id_editFacebookAdsAlertContactCustomValue .select2-chosen').text(data.name + ' (' + data.id + ')');
                                facebookAdsAccountsSelected = alertContactCustomValue["facebookAdsAccountsSelected"];
                            }
                        });
                        if (facebookAdsAccountsSelected && facebookAdsAccountsSelected.indexOf("act_") > -1) {
                            setFacebookAdsAccountsSelectOptions(alertContactID, facebookAdsAccountsSelected, alertContactValue);
                        } else if (facebookAdsAccounts.length == 1) {
                            $('#editFacebookAdsAlertContactCustomValue').trigger("change");
                            var value = $('#editFacebookAdsAlertContactCustomValue').val();
                            setFacebookAdsAccountsSelectOptions(alertContactID, value);
                        } else {
                            $('#editFacebookAdsAlertContactValue').val("");
                            $('#editFacebookAdsAlertContactValue').find("option").remove();
                    ...
                    $("#editFacebookAdsAlertContactValue").data("data-acid", alertContactID);
                    $("#editFacebookAdsAlertContactCustomValue").data("data-acid", alertContactID);
                }
                if (alertContactType == 23) {
                    $('#editAlertContactTypeTitle').html('Discord');
                    $('#editAlertContactType').val(alertContactType);
                    $('#editDiscordAlertContact').removeClass('hide');
                    $('#editDiscordAlertContactFriendlyName').val(alertContactFriendlyName);
                    $('#editDiscordAlertContactValue').html(alertContactValue);
                    $('#editDiscordAlertContactCustomValue').val(fRsi(alertContactCustomValue2));
                    $('#editDiscordAlertContactCustomValue2').val(fRsi(alertContactCustomValue));
                    if (alertContactSSLExpirationReminder == 1 || alertContactSSLExpirationReminder == "1") {
                        $("#editDiscordAlertContactSSLExpirationReminder").attr("checked", "checked");
                    } else {
                        $("#editDiscordAlertContactSSLExpirationReminder").removeAttr("checked");
                    }
                    $('#editDiscordAlertContactExcludeNotifications').select2("val", alertContactExclude);
                }
            }
        });
    });

    function setFacebookAdsAccountsSelectOptions(alertContactID, act, selecteds) {
        $.ajax({
            type: "GET",
            url: "inc/lib/FacebookAds/index.php?getAds=1&dacid=" + alertContactID + "&act=" + act,
            success: function(theResponse) {
                if (theResponse.type && theResponse.type == "error") {
                    $('#editFacebookAdsAlertContactValue').after('<div id="facebookAdsApiReachError" class="alert alert-error" style="clear: both;position: relative;top: 1rem;font-size: 0.75rem;line-height: 1.25em;margin: 0;">' + theResponse.text + "</div>");
                } else {
                    $.each(theResponse, function(id, details) {
                        var selected = false;
                        if (selecteds && selecteds.indexOf(details.id) > -1) {
                            selected = true;
                        }
                        var newOption = new Option("> " + details.name, details.id, selected, selected);
                        $('#editFacebookAdsAlertContactValue').append(newOption).trigger('change');
                        if (details.adsets) {
                            var adsets = details.adsets;
                            $.each(adsets, function(id, adset_details) {
                                var selected = false;
                                if (selecteds && selecteds.indexOf(adset_details.id) > -1) {
                                    selected = true;
                                }
                                var newOption = new Option(">> " + adset_details.name, adset_details.id, selected, selected);
                                $('#editFacebookAdsAlertContactValue').append(newOption).trigger('change');
                                var ads = adset_details.ads;
                                $.each(ads, function(id, ads_details) {
                                    var selected = false;
                                    if (selecteds && selecteds.indexOf(ads_details.id) > -1) {
                                        selected = true;
                                    }
                                    var newOption = new Option(">>> " + ads_details.name, ads_details.id, selected, selected);
                                    $('#editFacebookAdsAlertContactValue').append(newOption).trigger('change');
                                });
                            });
                        }
    ...
    var editAlertContactFormV = jQuery("#editAlertContactForm").validate({
            ...
            jQuery(form).ajaxSubmit({
                success: function(theResponse) {
                    if (theResponse == "true") {
                        ...
                        if ($("#editAlertContactType").val() == 1) {
                            var friendlyNameDiv = 'editMobileFreeAlertContactFriendlyName';
                        }
                        if ($("#editAlertContactType").val() == 2) {
                            var friendlyNameDiv = 'editEmailAlertContactFriendlyName';
                        }
                        if ($("#editAlertContactType").val() == 3) {
                            var friendlyNameDiv = 'editTwitterAlertContactFriendlyName';
                        }
                        if ($("#editAlertContactType").val() == 4) {
                            var friendlyNameDiv = 'editBoxcarAlertContactFriendlyName';
                        }
                        if ($("#editAlertContactType").val() == 5) {
                            var friendlyNameDiv = 'editWebHookAlertContactFriendlyName';
                            $('tr[data-alertcontactid="' + getParameterByName($("#editAlertContactForm").attr('action'), "alertContactID") + '"] span').attr('title', $('#editWebHookAlertContactValue').val());
                        }
                        if ($("#editAlertContactType").val() == 6) {
                            var friendlyNameDiv = 'editPushbulletAlertContactFriendlyName';
                        }
                        if ($("#editAlertContactType").val() == 7) {
                            var friendlyNameDiv = 'editZapierAlertContactFriendlyName';
                            $('tr[data-alertcontactid="' + getParameterByName($("#editAlertContactForm").attr('action'), "alertContactID") + '"] span').attr('title', $('#editZapierAlertContactValue').val());
                        }
                        if ($("#editAlertContactType").val() == 8) {
                            var friendlyNameDiv = 'editProSMSAlertContactFriendlyName';
                        }
                        if ($("#editAlertContactType").val() == 9) {
                            var friendlyNameDiv = 'editPushoverAlertContactFriendlyName';
                        }
                        if ($("#editAlertContactType").val() == 10) {
                            var friendlyNameDiv = 'editHipchatAlertContactFriendlyName';
                        }
                        if ($("#editAlertContactType").val() == 11) {
                            var friendlyNameDiv = 'editSlackAlertContactFriendlyName';
                        }
                        if ($("#editAlertContactType").val() == 12) {
                            var friendlyNameDiv = 'editIosAppAlertContactFriendlyName';
                        }
                        if ($("#editAlertContactType").val() == 13) {
                            var friendlyNameDiv = 'editAndroidAppAlertContactFriendlyName';
                        }
                        if ($("#editAlertContactType").val() == 14) {
                            var friendlyNameDiv = 'editProVoiceCallAlertContactFriendlyName';
                        }
                        if ($("#editAlertContactType").val() == 15) {
                            var friendlyNameDiv = 'editSplunkAlertContactFriendlyName';
                        }
                        if ($("#editAlertContactType").val() == 16) {
                            var friendlyNameDiv = 'editPagerDutyAlertContactFriendlyName';
                        }
                        if ($("#editAlertContactType").val() == 17) {
                            var friendlyNameDiv = 'editOpsGenieAlertContactFriendlyName';
                        }
                        if ($("#editAlertContactType").val() == 18) {
                            var friendlyNameDiv = 'editTelegramAlertContactFriendlyName';
                        }
                        if ($("#editAlertContactType").val() == 19) {
                            var friendlyNameDiv = 'editGoogleAdWordsAlertContactFriendlyName';
                        }
                        if ($("#editAlertContactType").val() == 20) {
                            var friendlyNameDiv = 'editMSTeamsAlertContactFriendlyName';
                        }
                        if ($("#editAlertContactType").val() == 21) {
                            var friendlyNameDiv = 'editHangoutsChatAlertContactFriendlyName';
                        }
                        if ($("#editAlertContactType").val() == 22) {
                            var friendlyNameDiv = 'editFacebookAdsAlertContactFriendlyName';
                        }
                        if ($("#editAlertContactType").val() == 23) {
                            var friendlyNameDiv = 'editDiscordAlertContactFriendlyName';
                        }
                        $('tr[data-alertcontactid="' + getParameterByName($("#editAlertContactForm").attr('action'), "alertContactID") + '"] span').html($('#' + friendlyNameDiv).val());
                    } else if (theResponse == "empty_adwords_campaigns") {
                        $("#editAlertContactNoGoogleAdwordsCampaignNotification").slideDown("slow");
                    } else if (theResponse == "empty_facebookads_campaigns") {
                        $("#editAlertContactNoFacebookAdsCampaignNotification").slideDown("slow");
                    } else {
        ...
        rules: {
            ...
            editMobileFreeAlertContactFriendlyName: {
                required: "#editAlertContactType[value=1]"
            },
            editMobileFreeAlertContactExcludeNotifications: {
                required: "#editAlertContactType[value=1]",
                max: {
                    depends: "#editAlertContactType[value=1]",
                    param: function() {
                        if ($('#editMobileFreeAlertContactSSLExpirationReminder').is(":checked")) {
                            return 3
                        } else {
                            return 2
                        }
            ...
            editMobileFreeAlertContactSSLExpirationReminder: {
                required: "#editMobileFreeAlertContactExcludeNotifications[value=1]"
            },
            editEmailAlertContactFriendlyName: {
                required: "#editAlertContactType[value=2]"
            },
            editEmailAlertContactExcludeNotifications: {
                required: "#editAlertContactType[value=2]",
                max: {
                    depends: "#editAlertContactType[value=2]",
                    param: function() {
            ...
            editEmailAlertContactSSLExpirationReminder: {
                required: "#editEmailAlertContactExcludeNotifications[value=2]"
            },
            editTwitterAlertContactFriendlyName: {
                required: "#editAlertContactType[value=3]"
            },
            editTwitterAlertContactExcludeNotifications: {
                required: "#editAlertContactType[value=3]",
                max: {
                    depends: "#editAlertContactType[value=3]",
                    param: function() {
                        if ($('#editTwitterAlertContactSSLExpirationReminder').is(":checked")) {
                            return 3
                        } else {
                            return 2
                        }
            ...
            editTwitterAlertContactSSLExpirationReminder: {
                required: "#editTwitterAlertContactExcludeNotifications[value=3]"
            },
            editBoxcarAlertContactFriendlyName: {
                required: "#editAlertContactType[value=4]"
            },
            editBoxcarAlertContactExcludeNotifications: {
                required: "#editAlertContactType[value=4]",
                max: {
                    depends: "#editAlertContactType[value=4]",
                    param: function() {
                        if ($('#editBoxcarAlertContactSSLExpirationReminder').is(":checked")) {
                            return 3
                        } else {
                            return 2
                        }
            ...
            editBoxcarAlertContactSSLExpirationReminder: {
                required: "#editBoxcarAlertContactExcludeNotifications[value=4]"
            },
            editWebHookAlertContactFriendlyName: {
                required: "#editAlertContactType[value=5]"
            },
            editWebHookAlertContactValue: {
                required: "#editAlertContactType[value=5]",
                url: "#editAlertContactType[value=5]",
                rangelength: [10, 1500]
            },
            editWebHookAlertContactPOSTValue: {
                checkJSON: "#editAlertContactType[value=5]",
                maxlength: 1300,
                required: function(element) {
                    if ($("#editWebHookAlertContactPOSTValueSendJSON").is(':checked')) {
                        return element.value == "";
                    } else {
                        return false;
                    }
            ...
            editWebHookAlertContactExcludeNotifications: {
                required: "#editAlertContactType[value=5]",
                max: {
                    depends: "#editAlertContactType[value=5]",
                    param: function() {
                        if ($('#editWebHookAlertContactSSLExpirationReminder').is(":checked")) {
                            return 3
                        } else {
                            return 2
                        }
            ...
            editWebHookAlertContactSSLExpirationReminder: {
                required: "#editWebHookAlertContactExcludeNotifications[value=5]"
            },
            editPushbulletAlertContactFriendlyName: {
                required: "#editAlertContactType[value=6]"
            },
            editPushbulletAlertContactValue: {
                required: "#editAlertContactType[value=6]"
            },
            editPushbulletAlertContactExcludeNotifications: {
                required: "#editAlertContactType[value=6]",
                max: {
                    depends: "#editAlertContactType[value=6]",
                    param: function() {
                        if ($('#editPushbulletAlertContactSSLExpirationReminder').is(":checked")) {
                            return 3
                        } else {
                            return 2
                        }
            ...
            editPushbulletAlertContactSSLExpirationReminder: {
                required: "#editPushbulletAlertContactExcludeNotifications[value=6]"
            },
            editZapierAlertContactFriendlyName: {
                required: "#editAlertContactType[value=7]"
            },
            editZapierAlertContactValue: {
                required: "#editAlertContactType[value=7]",
                url: "#editAlertContactType[value=7]"
            },
            editZapierAlertContactExcludeNotifications: {
                required: "#editAlertContactType[value=7]",
                max: {
                    depends: "#editAlertContactType[value=7]",
                    param: function() {
                        if ($('#editZapierAlertContactSSLExpirationReminder').is(":checked")) {
                            return 3
                        } else {
                            return 2
                        }
            ...
            editZapierAlertContactSSLExpirationReminder: {
                required: "#editZapierAlertContactExcludeNotifications[value=7]"
            },
            editProSMSAlertContactFriendlyName: {
                required: "#editAlertContactType[value=8]"
            },
            editProSMSAlertContactExcludeNotifications: {
                required: "#editAlertContactType[value=8]",
                max: {
                    depends: "#editAlertContactType[value=8]",
                    param: function() {
                        if ($('#editProSMSAlertContactSSLExpirationReminder').is(":checked")) {
                            return 3
                        } else {
                            return 2
                        }
            ...
            editProSMSAlertContactSSLExpirationReminder: {
                required: "#editProSMSAlertContactExcludeNotifications[value=8]"
            },
            editPushoverAlertContactFriendlyName: {
                required: "#editAlertContactType[value=9]"
            },
            editPushoverAlertContactValue: {
                required: "#editAlertContactType[value=9]"
            },
            editPushoverAlertContactExcludeNotifications: {
                required: "#editAlertContactType[value=9]",
                max: {
                    depends: "#editAlertContactType[value=9]",
                    param: function() {
                        if ($('#editPushoverAlertContactSSLExpirationReminder').is(":checked")) {
                            return 3
                        } else {
                            return 2
                        }
            ...
            editPushoverAlertContactSSLExpirationReminder: {
                required: "#editPushoverAlertContactExcludeNotifications[value=9]"
            },
            editHipchatAlertContactFriendlyName: {
                required: "#editAlertContactType[value=10]"
            },
            editHipchatAlertContactExcludeNotifications: {
                required: "#editAlertContactType[value=10]",
                max: {
                    depends: "#editAlertContactType[value=10]",
                    param: function() {
                        if ($('#editHipchatAlertContactSSLExpirationReminder').is(":checked")) {
                            return 3
                        } else {
                            return 2
                        }
            ...
            editHipchatAlertContactSSLExpirationReminder: {
                required: "#editHipchatAlertContactExcludeNotifications[value=10]"
            },
            editSlackAlertContactFriendlyName: {
                required: "#editAlertContactType[value=11]"
            },
            editSlackAlertContactExcludeNotifications: {
                required: "#editAlertContactType[value=11]",
                max: {
                    depends: "#editAlertContactType[value=11]",
                    param: function() {
                        if ($('#editSlackAlertContactSSLExpirationReminder').is(":checked")) {
                            return 3
                        } else {
                            return 2
                        }
            ...
            editSlackAlertContactSSLExpirationReminder: {
                required: "#editSlackAlertContactExcludeNotifications[value=11]"
            },
            editSplunkAlertContactFriendlyName: {
                required: "#editAlertContactType[value=15]"
            },
            editSplunkAlertContactValue: {
                required: "#editAlertContactType[value=15]",
                url: "#editAlertContactType[value=15]"
            },
            editSplunkAlertContactExcludeNotifications: {
                required: "#editAlertContactType[value=15]",
                max: {
                    depends: "#editAlertContactType[value=15]",
                    param: function() {
                        if ($('#editSplunkAlertContactSSLExpirationReminder').is(":checked")) {
                            return 3
                        } else {
                            return 2
                        }
            ...
            editSplunkAlertContactSSLExpirationReminder: {
                required: "#editSplunkAlertContactExcludeNotifications[value=15]"
            },
            editPagerDutyAlertContactFriendlyName: {
                required: "#editAlertContactType[value=16]"
            },
            editPagerDutyAlertContactValue: {
                required: "#editAlertContactType[value=16]"
            },
            editPagerDutyAlertContactExcludeNotifications: {
                required: "#editAlertContactType[value=16]",
                max: {
                    depends: "#editAlertContactType[value=16]",
                    param: function() {
                        if ($('#editPagerDutyAlertContactSSLExpirationReminder').is(":checked")) {
                            return 3
                        } else {
                            return 2
                        }
            ...
            editPagerDutyAlertContactSSLExpirationReminder: {
                required: "#editPagerDutyAlertContactExcludeNotifications[value=16]"
            },
            editOpsGenieAlertContactFriendlyName: {
                required: "#editAlertContactType[value=17]"
            },
            editOpsGenieAlertContactValue: {
                required: "#editAlertContactType[value=17]",
                url: "#editAlertContactType[value=17]"
            },
            editOpsGenieAlertContactExcludeNotifications: {
                required: "#editAlertContactType[value=17]",
                max: {
                    depends: "#editAlertContactType[value=17]",
                    param: function() {
                        if ($('#editOpsGenieAlertContactSSLExpirationReminder').is(":checked")) {
                            return 3
                        } else {
                            return 2
                        }
            ...
            editOpsGenieAlertContactSSLExpirationReminder: {
                required: "#editOpsGenieAlertContactExcludeNotifications[value=17]"
            },
            editProVoiceCallAlertContactFriendlyName: {
                required: "#editAlertContactType[value=14]"
            },
            editProVoiceCallAlertContactExcludeNotifications: {
                required: "#editAlertContactType[value=14]",
                max: {
                    depends: "#editAlertContactType[value=14]",
                    param: function() {
                        if ($('#editProVoiceCallAlertContactSSLExpirationReminder').is(":checked")) {
                            return 3
                        } else {
                            return 2
                        }
            ...
            editProVoiceCallAlertContactSSLExpirationReminder: {
                required: "#editProVoiceCallAlertContactExcludeNotifications[value=14]"
            },
            editIosAppAlertContactFriendlyName: {
                required: "#editAlertContactType[value=12]"
            },
            editIosAppAlertContactExcludeNotifications: {
                required: "#editAlertContactType[value=12]",
                max: {
                    depends: "#editAlertContactType[value=12]",
                    param: function() {
                        if ($('#editIosAppAlertContactSSLExpirationReminder').is(":checked")) {
                            return 3
                        } else {
                            return 2
                        }
            ...
            editIosAppAlertContactSSLExpirationReminder: {
                required: "#editIosAppAlertContactExcludeNotifications[value=12]"
            },
            editAndroidAppAlertContactFriendlyName: {
                required: "#editAlertContactType[value=13]"
            },
            editAndroidAppAlertContactExcludeNotifications: {
                required: "#editAlertContactType[value=13]",
                max: {
                    depends: "#editAlertContactType[value=13]",
                    param: function() {
                        if ($('#editAndroidAppAlertContactSSLExpirationReminder').is(":checked")) {
                            return 3
                        } else {
                            return 2
                        }
            ...
            editAndroidAppAlertContactSSLExpirationReminder: {
                required: "#editAndroidAppAlertContactExcludeNotifications[value=13]"
            },
            editTelegramAlertContactFriendlyName: {
                required: "#editAlertContactType[value=18]"
            },
            editTelegramAlertContactExcludeNotifications: {
                required: "#editAlertContactType[value=18]",
                max: {
                    depends: "#editAlertContactType[value=18]",
                    param: function() {
                        if ($('#editTelegramAlertContactSSLExpirationReminder').is(":checked")) {
                            return 3
                        } else {
                            return 2
                        }
            ...
            editTelegramAlertContactSSLExpirationReminder: {
                required: "#editTelegramAlertContactExcludeNotifications[value=18]"
            },
            editGoogleAdWordsAlertContactFriendlyName: {
                required: "#editAlertContactType[value=19]"
            },
            editMSTeamsAlertContactFriendlyName: {
                required: "#editAlertContactType[value=20]",
                maxlength: 30
            },
            editMSTeamsAlertContactExcludeNotifications: {
                required: "#editAlertContactType[value=20]",
                max: {
                    depends: "#editAlertContactType[value=20]",
                    param: function() {
                        if ($('#editMSTeamsAlertContactSSLExpirationReminder').is(":checked")) {
                            return 3
                        } else {
                            return 2
                        }
            ...
            editMSTeamsAlertContactSSLExpirationReminder: {
                required: "#editMSTeamsAlertContactExcludeNotifications[value=20]"
            },
            editHangoutsChatAlertContactFriendlyName: {
                required: "#editAlertContactType[value=21]",
                maxlength: 30
            },
            editHangoutsChatAlertContactRoomURL: {
                required: "#editAlertContactType[value=21]",
                url: "#editAlertContactType[value=21]"
            },
            editHangoutsChatAlertContactExcludeNotifications: {
                required: "#editAlertContactType[value=21]",
                max: {
                    depends: "#editAlertContactType[value=21]",
                    param: function() {
                        if ($('#editHangoutsChatAlertContactSSLExpirationReminder').is(":checked")) {
                            return 3
                        } else {
                            return 2
                        }
            ...
            editHangoutsChatAlertContactSSLExpirationReminder: {
                required: "#editHangoutsChatAlertContactExcludeNotifications[value=3]"
            },
            editFacebookAdsAlertContactFriendlyName: {
                required: "#editAlertContactType[value=22]"
            },
            editDiscordAlertContactFriendlyName: {
                required: "#editAlertContactType[value=23]"
            },
            editDiscordAlertContactExcludeNotifications: {
                required: "#editAlertContactType[value=23]",
                max: {
                    depends: "#editAlertContactType[value=23]",
                    param: function() {
                        if ($('#editDiscordAlertContactSSLExpirationReminder').is(":checked")) {
                            return 3
                        } else {
                            return 2
                        }
            ...
            editDiscordAlertContactSSLExpirationReminder: {
                required: "#editDiscordAlertContactExcludeNotifications[value=23]"
            },
        },
        messages: {
            editAlertContactType: {
            ...
            editPagerDutyAlertContactValue: {
                required: "PagerDuty integration key is required"
            },
            editPagerDutyAlertContactExcludeNotifications: {
                required: "Events to be received is required",
                max: "Event or SSL notifications must be selected"
            ...
            editProVoiceAlertContactExcludeNotifications: {
                required: "Events to be received is required",
                max: "Event or SSL notifications must be selected"
            },
            editProVoiceAlertContactSSLExpirationReminder: {
                required: "SSL or event notifications must be selected"
            },
            editIosAppAlertContactFriendlyName: {
                required: "Friendly name is required"
    ...
    $(document).on("click", '.editBaseAccountEmailAlertContact', function(e) {
        ...
        $('#editPagerDutyAlertContact').addClass('hide');
        $('#editOpsGenieAlertContact').addClass('hide');
        $('#editAlertContactForm').attr('action', 'inc/dml/userDML.php?action=editBaseAccountEmailAlertContact&userID=' + userID);
        $("#editAlertContactSuccessNotification").slideUp();
        $('#editHipchatAlertContactCustomValue').val("");
        $('#editSlackAlertContactCustomValue').val("");
        $('#editTelegramAlertContactCustomValue').val("");
        $('#editBaseAccountAlertContactExcludeNotifications').select2("val", "");
        $("#editProVoiceCallAlertContactValue").val("");
        $("#editProVoiceCallAlertContactFriendlyName").val("");
        $("#editMobileFreeAlertContactSSLExpirationReminder").removeAttr("checked");
        ...
        $('#editAlertContactType').val(0);
        $('#editBaseAccountAlertContact').removeClass('hide');
        $('#editGoogleAdWordsAlertContactCustomValue2').val("");
        $('#editGoogleAdWordsAlertContactValue').find("option").remove();
        $('#editGoogleAdWordsAlertContactValue').trigger("change");
        $('#editGoogleAdWordsAlertContactCustomValue').find("option").remove();
        $('#editGoogleAdWordsAlertContactCustomValue').trigger("change");
        $('#editFacebookAdsAlertContactValue').val("");
        $('#editFacebookAdsAlertContactValue').find("option").remove();
        $('#editFacebookAdsAlertContactValue').trigger("change");
        $('#editFacebookAdsAlertContactCustomValue').val("");
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                if (userEmailSSLExpirationReminderStatus == 1 || userEmailSSLExpirationReminderStatus == "1") {
                    $("#editBaseAccountAlertContactSSLExpirationReminder").attr("checked", "checked");
                } else {
                    $("#editBaseAccountAlertContactSSLExpirationReminder").removeAttr("checked");
                }
                $('#editBaseAccountAlertContactExcludeNotifications').select2("val", response.user.userNotificationExclude);
            },
    ...
    $('#editGoogleAdWordsAlertContactCustomValue').select2({
        closeOnSelect: false,
        ...
        ajax: {
            url: "inc/lib/GoogleAdWords/index.php",
            dataType: 'json',
            quietMillis: 100,
            data: function() {
                return {
                    get_ccl: 1,
                    dacid: $('#editGoogleAdWordsAlertContactCustomValue').data('data-acid')
                };
            },
            results: function(data) {
                return {
    ...
    $("#editSelectedGoogleAdWordsAlertContactValue").select2({
    ...
    $("#editSelectedGoogleAdWordsAlertContactValue").on("select2-opening", function(e) {
        e.preventDefault();
    });
    $(document).on("change", "#editGoogleAdWordsAlertContactCustomValue", function() {
        var activateCampaign = $(this).val() != null;
        $('#editGoogleAdWordsAlertContactValue').select2('enable', activateCampaign);
        $("#editGoogleAdWordsAlertContactValue").val("").trigger("change");
    });
    $('#editGoogleAdWordsAlertContactValue').select2({
        closeOnSelect: false,
        minimumInputLength: 0,
        ...
        ajax: {
            ...
            data: function(params) {
                var query = {
                    get_cccl: $('#editGoogleAdWordsAlertContactCustomValue').val() + '-' + $('#editGoogleAdWordsAlertContactCustomValue2').val(),
                    dacid: $('#editGoogleAdWordsAlertContactValue').data('data-acid')
                }
                return query;
            },
            results: function(data) {
            ...
            success: function(response) {
                if (response.length === 0) {
                    $('#adwordsNotification').notyfy({
                        'text': 'No campaign found connected to this AdWords Client',
                        'type': 'error'
                    });
                }
            }
        ...
        formatSelection: function(data) {
            var selectionHolder = $('#editSelectedGoogleAdWordsAlertContactValue');
            if (selectionHolder.find("option[value='" + data.id + "']").length) {
                selectionHolder.val(data.id).trigger('change');
            } else {
                var newOption = new Option(data.text, data.id, true, true);
                selectionHolder.append(newOption).trigger('change');
            }
            return data.text;
        }
    });
